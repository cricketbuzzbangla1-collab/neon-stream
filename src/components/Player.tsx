...
const Player = ({ channel }: PlayerProps) => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case "m":
          ...
          break;
        case "arrowleft":
          if (!isLiveStream) { video.currentTime = Math.max(0, video.currentTime - 10); }
          break;
        case "arrowright":
          if (!isLiveStream) { video.currentTime = Math.min(video.duration, video.currentTime + 10); }
          break;
      }
      resetControlsTimer();
    };
  ...
  }, [isLiveStream, resetControlsTimer]);
  ...
  const initShaka = useCallback(async () => {
    ...
    try {
      ...
      if (!shaka.default.Player.isBrowserSupported()) {
        console.warn("Shaka not supported, falling back to native");
      ...
      player.addEventListener("adaptation", () => {
        // Update quality levels from active variant tracks
  ...
  useEffect(() => {
    ...
    if (currentMode === "dash") {
      initShaka();
    } else if (currentMode === "hls" || currentMode === "hls-retry") {
      if (Hls.isSupported()) {
        const hls = new Hls({
          enableWorker: true, lowLatencyMode: true, startLevel: -1,
          ...(currentMode === "hls-retry" ? { fragLoadingMaxRetry: 10, manifestLoadingMaxRetry: 10, levelLoadingMaxRetry: 10 } : {}),
        });
        hlsRef.current = hls;
        hls.loadSource(url);
        ...
        hls.on(Hls.Events.ERROR, (_, data) => {
          if (data.fatal) {
            if (currentMode === "hls") setCurrentMode("hls-retry");
    ...
    >
      ...
      {error && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/80 gap-4 z-10">
          ...
          <button
            onClick={() => { setError(false); retryCountRef.current = 0; setCurrentMode(channel.playerType || "hls"); }}
      ...
      {seekIndicator && (
        <div className={`absolute top-1/2 -translate-y-1/2 z-20 pointer-events-none animate-fade-in ${seekIndicator.side === "left" ? "left-12" : "right-12"}`}>
      ...
      {isLiveStream && !error && (
        <div className="absolute top-3 left-3 z-20">
          <button onClick={goLive} className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-destructive/90 backdrop-blur-sm text-destructive-foreground text-[11px] font-bold uppercase tracking-wider">
      ...
      {/* Player type badge */}
      {currentMode === "dash" && !error && (
        <div className="absolute top-3 right-3 z-20">
          <span className="px-2 py-0.5 rounded-md bg-accent/80 backdrop-blur-sm text-accent-foreground text-[10px] font-bold uppercase tracking-wider">
            DASH
          </span>
        </div>
      )}
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-background/80 via-background/40 to-transparent pt-16 pb-3 px-3 space-y-2">
          ...
          <div className="flex items-center justify-between gap-1">
            ...
            <div className="flex items-center gap-1">
              {/* Playback speed */}
              <div className="relative">
                <button onClick={() => { setShowSpeedMenu(!showSpeedMenu); setShowQualityMenu(false); }} className="p-2 rounded-lg hover:bg-foreground/10 text-foreground transition-colors">
                  <Gauge className="w-5 h-5" />
                </button>
                {showSpeedMenu && (
                  <div className="absolute bottom-full right-0 mb-2 bg-card/95 backdrop-blur-xl border border-border rounded-lg overflow-hidden shadow-2xl min-w-[100px] z-30">
                    <div className="p-1">
                      {PLAYBACK_SPEEDS.map((s) => (
                        <button
                          key={s}
                          onClick={() => handleSpeedChange(s)}
                          className={`w-full text-left px-3 py-2 text-xs rounded-md transition-colors ${playbackSpeed === s ? "bg-primary text-primary-foreground font-bold" : "text-foreground hover:bg-foreground/10"}`}
                        >
                          {s}x
                        </button>
                      ))}
              ...
              {qualityLevels.length > 0 && (
                <div className="relative">
                  <button onClick={() => { setShowQualityMenu(!showQualityMenu); setShowSpeedMenu(false); }} className="p-2 rounded-lg hover:bg-foreground/10 text-foreground transition-colors">
                    ...
                    <div className="absolute bottom-full right-0 mb-2 bg-card/95 backdrop-blur-xl border border-border rounded-lg overflow-hidden shadow-2xl min-w-[140px] z-30">
                      <div className="p-1">
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 text-xs rounded-md transition-colors ${currentQuality === -1 ? "bg-primary text-primary-foreground font-bold" : "text-foreground hover:bg-foreground/10"}`}
                        >
                          Auto
                        </button>
                        {qualityLevels.map((level) => (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 text-xs rounded-md transition-colors ${currentQuality === level.index ? "bg-primary text-primary-foreground font-bold" : "text-foreground hover:bg-foreground/10"}`}
                          >
              ...
              {/* Fill/Contain toggle */}
              <button onClick={() => setObjectFit(objectFit === "contain" ? "cover" : "contain")} className="p-2 rounded-lg hover:bg-foreground/10 text-foreground transition-colors" title={objectFit === "contain" ? "Fill" : "Fit"}>
